...
- Use each TC-specific command for normal headless execution. Launch visible UI only with `npm.cmd run test:headed`; it automatically discovers numbered TC specs, runs every TC sequentially by TC number, and performs their real side effects.
