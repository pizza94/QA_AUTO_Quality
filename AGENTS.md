...
- For TC-006, use `npm.cmd run test:profiling-settings` for normal headless execution. Launch visible UI only with the explicit user-facing command `npm.cmd run test:profiling-settings:headed`; this headed command performs the same real save operations.
