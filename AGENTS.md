...
- After TC-010 immediately executes the TC-009 job, click the scoped execution-search button every 10 seconds until that exact row has a new start time, an end time, and 100% progress; then verify every TC-006 column ID is complete in the monitoring tab.
